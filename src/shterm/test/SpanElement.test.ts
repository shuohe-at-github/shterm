...
import { ShTerm, SpanElement, TextSpan } from '..'

describe('shterm.SpanElement', () => {
    ...
    describe('::create($term: ShTerm, span: TextSpan): SpanElement', () => {
        it('参数异常：span.text 不能是空字符串', () => {
            expect(() => {
                SpanElement.create($term!, TextSpan.create('', {}, $term!.options))
            }).to.throw()
        })

        it('默认英文字体', () => {
            const $span = SpanElement.create($term!, TextSpan.create('abc', {}, $term!.options))
        ...
        it('指定其他字体', () => {
            const $span = SpanElement.create($term!, TextSpan.create('中国', { font: new shlib.Font('Arial', 20) }, $term!.options))
